...
const RiskAnalytics = () => {
    ...
    const getMapColor = (state) => {
        ...
        if (activeLayer === 'census') {
            const val = state.censusCoverage || 0;
            if (val >= 100) return 'rgba(0, 255, 255, 0.7)'; // Saturated
            if (val >= 95) return 'rgba(0, 255, 136, 0.6)'; // High
            if (val >= 90) return 'rgba(255, 204, 0, 0.6)'; // Moderate
            return 'rgba(255, 50, 50, 0.6)'; // Low
        }
    ...
    const layers = [
        ...
        // { id: 'load', label: 'System Load', icon: <Activity size={14} /> },
        { id: 'census', label: 'Census', icon: <Users size={14} /> },
        { id: 'readiness', label: 'Availability', icon: <TrendingUp size={14} /> },
        { id: 'maturity', label: 'Productivity', icon: <Activity size={14} /> },
                            ...
                            <div className="map-hover-stats" style={{
                                ...
                                <div className="tooltip-header">
                                    ...
                                    <div className="system-status-badge" style={{
                                        backgroundColor: activeLayer === 'risk' ? (activeState.risk > 7 ? 'rgba(255, 0, 0, 0.6)' : 'rgba(0, 255, 136, 0.6)') :
                                            activeLayer === 'census' ? (activeState.censusCoverage > 95 ? 'rgba(0, 255, 255, 0.6)' : 'rgba(255, 100, 0, 0.6)') :
                                                'rgba(0, 150, 255, 0.6)'
                                    }}>
                                        {activeLayer === 'risk' ? (activeState.risk > 7 ? 'CRITICAL' : 'STABLE') :
                                            activeLayer === 'census' ? (activeState.censusCoverage > 95 ? 'SATURATED' : 'SKEWED') :
                                                'OPERATIONAL'}
                                    </div>
                                </div>
                                <div className="stat-row">
                                    ...
                                    <span className="val highlight">
                                        ...
                                        {activeLayer === 'census' && `${activeState.censusCoverage}% Saturation`}
