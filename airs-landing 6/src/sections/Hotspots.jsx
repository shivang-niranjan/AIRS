...
import { CloudRain, Activity, ShieldAlert, Cpu, Shield } from 'lucide-react';
...
const Hotspots = () => {
    ...
    const getModeColor = (d) => {
        ...
        if (viewMode === 'silent') {
            return d.isSilent ? '#bf00ff' : 'rgba(0, 255, 136, 0.2)';
        }
        if (viewMode === 'stress') {
            ...
            <div className="hotspots-content">
                <div className="text-side animate-on-scroll">
                    ...
                    <div className="map-controls">
                        ...
                        <button
                            className={`control-btn ${viewMode === 'silent' ? 'active' : ''}`}
                            onClick={() => setViewMode('silent')}
                        >
                            <Shield size={16} /> Silent Zones
                        </button>
                    </div>
                                ...
                                {viewMode === 'silent' && (
                                    <>
                                        <div className="detail-row">
                                            ...
                                            <span className="score" style={{ color: getModeColor(activeDistrict) }}>
                                                {activeDistrict.isSilent ? 'SILENT (DATA GAP)' : 'ACTIVE'}
                                            </span>
                                        </div>
                                        {activeDistrict.isSilent && (
                                            <div className="detail-row">
                                                <span>Detection:</span>
                                                <span className="action">SIGNAL ABSENCE</span>
                                            </div>
                                        )}
                                    </>
                                )}
